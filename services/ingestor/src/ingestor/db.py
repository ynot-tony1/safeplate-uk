...
def _bulk_insert_rating_changes(conn: psycopg.Connection, changes: Sequence[dict[str, object]]) -> None:
    row_placeholder = "(" + ",".join(["%s"] * len(_RATING_CHANGE_COLUMNS)) + ")"
    values_sql = sql.SQL(", ").join(sql.SQL(row_placeholder) for _ in changes)
    columns_sql = sql.SQL(", ").join(sql.Identifier(c) for c in _RATING_CHANGE_COLUMNS)
...
def create_ingestion_run(
    ...
    *,
    ...
    with conn.cursor() as cur:
        cur.execute(
            """
            INSERT INTO ingestion_runs (status, workflow_run_id, git_sha)
            VALUES ('RUNNING', %s, %s)
            RETURNING id
            """,
            (workflow_run_id, git_sha),
        )
        row = cur.fetchone()
